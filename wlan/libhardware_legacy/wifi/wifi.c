...
{
    /*
     * WifiMonitor.java expects IFNAME=<intrface>
     * for events belonging to a non-p2p interface.
     * Add IFNAME to avoid WifiMonitor dropping the events
     * as wpa_supplicant_rt does not provide it.
     */
    int res = wifi_wait_on_socket(buf, buflen);

    const char* ifname = primary_iface;
    size_t ifname_len = strlen(ifname);
    size_t buf_offset = IFNAMELEN + ifname_len + 1;

    if(ifname_len > 0 && buf_offset < buflen) {
        // move event string
        memmove(buf + buf_offset, buf, buflen - buf_offset);
        // copy IFNAME=
        memcpy(buf, IFNAME, IFNAMELEN);
        // copy <interface>
        memcpy(buf + IFNAMELEN, ifname, ifname_len);
        // add seperating space
        buf[buf_offset - 1] = ' ';
        // ensure that cmd string ends - even when cut off
        buf[buflen - 1] = '\0';
    }

    ALOGD("Got event '%s'", buf);
    return res;
}
...
{
    /*
     * WifiNative.java issues commands in the format
     * 
     *  IFNAME=<interface> <cmd>
     *
     * Strip IFNAME part because wpa_supplicant_8_rt 
     * does not support it.
     */
    const char* ifname_end = strchr(command, ' ');
    int res;
    
    if(ifname_end && strncmp(command, IFNAME, IFNAMELEN) == 0) {
        const char* cmd_begin = ifname_end + 1;

        ALOGD("Sending stripped version '%s' of command '%s'", cmd_begin, command);
        res = wifi_send_command(cmd_begin, reply, reply_len);
    } else {
        ALOGD("Sending command '%s'", command);
        res = wifi_send_command(command, reply, reply_len);
    }

    ALOGD("Got response '%s'", reply);
    return res;
}
